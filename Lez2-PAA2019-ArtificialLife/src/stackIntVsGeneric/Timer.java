...
public class Timer {
    ...
    /**
     * Starts the timer. Once started the timer cannot be start again, but
     * {@link Timer#stop() stop} must be used before
     */
    public void start() {
	if (!started) {
	    started = true;
    ...
    /**
     * Stops the timer. Only a timer that was started with {@link Timer#start()
     * start} can be stopped.
     */
    public void stop() {
	if (started) {
	    started = false;
